...
### Task 13: 상단 고정 툴바 (컨플식) — 2026-07-17 사용자 요청 추가

> 실행 순서: Task 8(플로팅 툴바) 완료 후. ToolbarButtons의 커맨드 패턴을 재사용한다.
> **정렬(왼쪽/가운데/오른쪽)은 범위 밖** — GFM 마크다운에 텍스트 정렬 문법이 없고, HTML 스타일 주입은
> 저장 포맷 결정 위반 + MarkdownView가 raw HTML을 렌더하지 않아 보기 화면에서 무시된다.
> 로드맵 3단계 ⚠️(밑줄·글자색과 같은 그룹)로 이관 — 로드맵/스펙 문서에 반영 완료.

**Files:**
- Create: `src/features/wiki/editor/components/TopToolbar.tsx`
- Modify: `src/features/wiki/editor/WikiEditor.tsx` (에디터 위에 툴바 렌더)
- Modify: `src/app/app.css`
- Test: `src/features/wiki/editor/components/TopToolbar.test.tsx`

**Interfaces:**
- Consumes: WikiEditor의 `editor` 인스턴스 (Task 3), Task 8의 chain 커맨드 패턴
- Produces: `TopToolbar({ editor }: { editor: Editor })`

- [ ] **Step 1: 실패하는 테스트 작성**

```tsx
// TopToolbar.test.tsx — Editor 인스턴스 직접 생성 (BubbleToolbar.test.tsx와 동일 패턴)
import { describe, expect, it } from "vitest";
import { render, screen } from "@testing-library/react";
import userEvent from "@testing-library/user-event";
import { Editor } from "@tiptap/core";
...
Run: `npx vitest run src/features/wiki/editor/components/TopToolbar.test.tsx`
Expected: FAIL — 모듈 없음
...
WikiEditor JSX: `<div className="wiki-editor">` 안, EditorContent 위에 `{editor && <TopToolbar editor={editor} />}`.
...
.top-toolbar button:disabled { opacity: 0.4; cursor: default; }
.top-toolbar button.is-active {
  background: var(--color-background-accent-blue-subtlest, #e9f2ff);
  color: var(--color-text-accent-blue, #0c66e4);
}
.top-toolbar select {
...
```bash
git add src/features/wiki/editor src/app/app.css
git commit -m "feat(editor): 상단 고정 툴바 — 블록 타입·서식·목록·삽입, 마크다운 호환 컨트롤만"
```
...
### Task 19: 사이드바 컨플식 개편 — 접기 + 너비 조절 (2026-07-17 사용자 요청 추가)

> 실행 순서: Task 18 다음, Task 12(최종 스윕) 직전.
> 컨플루언스 사이드바 대응: ① 접기/펼치기(닫기 버튼 + 접힌 상태의 열기 버튼), ② 드래그 너비 조절,
> ③ 영역 구조화(헤더=스페이스 선택 / 본문=검색+트리 스크롤 / 푸터=새 페이지·스페이스 만들기).
> 설정은 T18과 같은 패턴으로 localStorage (`wiki.ui.sidebar.collapsed`, `wiki.ui.sidebar.width`) — 전역(페이지 무관).

**Files:**
- Create: `src/features/wiki/lib/sidebarPrefs.ts` — `getSidebarPrefs(): { collapsed: boolean; width: number }`, `setSidebarCollapsed(v)`, `setSidebarWidth(px)`, `clampSidebarWidth(px)` (min 200 / max 480 / 기본 288, 예외 안전), `useSidebarPrefs()` 훅
- Create: `src/features/wiki/components/SidebarResizer.tsx` — 사이드바 우측 경계 세로 핸들. pointerdown→pointermove로 폭 계산(`clampSidebarWidth`), pointerup에서 저장. `role="separator"` `aria-orientation="vertical"` `aria-label="사이드바 너비 조절"`, 키보드 ←/→ 16px 단위 조절
- Modify: `src/features/wiki/components/WikiLayout.tsx` — aside 구조를 3영역(`.wiki-sidebar-header`(Select+닫기 버튼)/`.wiki-sidebar-body`(검색+트리, overflow-y auto)/`.wiki-sidebar-footer`(새 페이지+SpaceCreateModal))으로 재편. `collapsed`면 aside 미렌더 + `.wiki-sidebar-opener`(고정 위치 열기 버튼, aria-label "사이드바 열기") 표시. 닫기 버튼 aria-label "사이드바 접기", aria-expanded. aside 인라인 `style={{ width }}`
- Modify: `src/app/app.css` — 3영역 레이아웃(flex column, body만 스크롤), 리사이저 핸들(4px, hover 하이라이트), 접힘 상태
- Test: `sidebarPrefs.test.ts` (clamp/기본값/저장·복원/스토리지 예외), `App.w5-sidebar.test.tsx` 통합 (접기 → 트리 사라짐+열기 버튼, 다시 열기, 새로고침 후 유지(localStorage), 리사이저 키보드 조절 → aside width 스타일 변경). 드래그 좌표는 jsdom 제외 — clamp 유닛으로 커버

**Interfaces:** 기존 PageTree/검색/스페이스 셀렉트 동작 무변경 — 배치만 재편. Outlet 컨텍스트 무변경.

TDD 사이클 후 커밋 `feat(sidebar): 컨플식 사이드바 — 접기/열기·드래그 너비 조절·3영역 구조화`.

### Task 20: 편집↔보기 시각 동등화 (2026-07-17 사용자 피드백 — T19보다 먼저 실행)

> 사용자 피드백: "에디터랑 저장 후가 완전히 같아야 하는데 … /로 넣는 것들이 편집모드에서는 안 보여."
> WYSIWYG의 핵심 약속이므로 우선 처리. 세 갈래:

**Files:**
- Modify: `src/app/app.css` — ① 폭 일치: `.page-edit` 기본을 `.page-view`와 동일한 720px 중앙 정렬로 (T18의 880px 유지 결정 폐기, `--full` 변형은 유지). ② 타이포 일치: `.markdown-body`의 요소 규칙(제목 크기/목록/인용/표/코드/구분선 여백)을 `.markdown-body, .wiki-editor .ProseMirror` 공용 셀렉터로 재편 — 중복 정의 제거, 한쪽 수정 시 양쪽 반영 구조. ③ 에디터 내 패널 4색 스타일(`.wiki-editor .md-alert-*` — 뷰와 동일 변수 재사용).
- Create: `src/features/wiki/editor/extensions/alertDecoration.ts` — blockquote 첫 텍스트가 `[!TYPE]`(이스케이프 형태 포함)로 시작하면 해당 blockquote 노드에 `md-alert md-alert-<type>` 클래스 데코레이션 부여 + 마커 텍스트 범위에 `md-alert-marker` 인라인 데코레이션(작은 배지 스타일 — 마커는 편집 가능해야 하므로 숨기지 않고 배지화). wikiLink 부재 데코레이션(Task 2)과 동일한 Plugin 패턴.
- Modify: `src/features/wiki/editor/extensions/base.ts` 또는 `WikiEditor.tsx` — alertDecoration 등록 (편집 전용이므로 WikiEditor 권장).
- Test: `alertDecoration.test.ts` (5타입 클래스 부여/비마커 무데코/마커 배지 범위 — Editor 인스턴스 + jsdom DOM 검사, Task 2 데코 테스트 패턴), App 통합 1개 (편집 진입 → `> [!NOTE]` 포함 본문 → `.md-alert-note` 클래스 존재), 폭 일치 회귀 (App.w5-width 갱신 — 편집 화면 기본 720px 클래스).

**범위 밖 (로드맵 노트)**: 에디터 내 코드 하이라이팅(CodeBlockLowlight 통합)은 별도 후속 — 이번엔 패널·폭·타이포만.

TDD 사이클 후 커밋 `feat(editor): 편집↔보기 시각 동등화 — 패널 라이브 렌더·폭 통일·타이포 공용화`.
