...
| 페이지/폴더 트리와 계층 이동 | ✅ | 드롭 의도(앞/뒤/하위) UX·이동 다이얼로그·서버 형제 순서(V9 move, P1-001 해소) 완료(2026-08-23) |
| 페이지 복제 | ⚠️ | v1 완료(2026-08-23): 단일 페이지+첨부 객체 복사+본문 참조 재작성. 하위 포함 복제·권한 복제는 후속 |
...
| 전역/본문/첨부 검색 | ✅ | 기능 브랜치 main 통합 완료(2026-08-23, P1-007). 페이지 제한 누출 테스트는 W18에서 |
...
- ~~`WIKI-P1-001`~~: 해소(2026-08-23) — V9 sort_order + POST /move(부모·순서 한 트랜잭션,
  version 불변, 스페이스 행 잠금 직렬화).
...
- ~~`WIKI-P1-007`~~ 통합 완료(2026-08-23). 페이지 제한 누출 테스트는 W18 제한 도입과 함께.
...
- **페이지 제한 완료(2026-08-24)**: V12 모델 + effective permission 전 지점(본문/트리/첨부/
  댓글/티켓/이동/리비전) + 관리 API·자물쇠 다이얼로그 + 이동 영향 2단계 + 검색 gRPC 후필터
  (P1-007 잔여 해소). 누출 테스트 매트릭스 통과. 잔여: 휴지통·라벨·백링크·watch·감사 이벤트
...
- `wiki-front`: 95개 테스트 파일, 683개 테스트 통과(라이브 1개 별도), 기능 플래그 OFF/ON production build 통과
- `wiki-backend`: 27개 suite, 151개 테스트 통과(실제 PostgreSQL Flyway V1→V9 포함)
