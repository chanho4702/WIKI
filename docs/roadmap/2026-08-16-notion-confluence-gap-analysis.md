...
| 페이지/폴더 트리와 계층 이동 | ⚠️ | 드롭 의도(앞/뒤/하위) UX·이동 다이얼로그 완료(2026-08-23). 형제 순서는 여전히 서버 계약에 없음(P1-001) |
...
- `wiki-front`: 95개 테스트 파일, 680개 테스트 통과(라이브 1개 별도), 기능 플래그 OFF/ON production build 통과
- `wiki-backend`: 26개 suite, 145개 테스트 통과(실제 PostgreSQL Flyway V1→V8 포함)
