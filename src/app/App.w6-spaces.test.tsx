...
describe("W6 스페이스 플라이아웃", () => {
  it("트리거를 열고 필터한 뒤 항목을 클릭하면 해당 스페이스로 이동하고 패널이 닫힌다", async () => {
    ...
    renderApp();
    const trigger = await screen.findByRole("button", { name: "개발 위키 (DEV)" });
    ...
    // 트리거도 새 스페이스로 바뀐다
    expect(await screen.findByRole("button", { name: "운영 위키 (OPS)" })).toBeInTheDocument();
  });

  it("별표를 누르면 '별표 표시됨' 섹션으로 옮겨가고, 현재 스페이스는 그 섹션에서 제외된다", async () => {
    const user = userEvent.setup();
    renderApp();
    await user.click(await screen.findByRole("button", { name: "개발 위키 (DEV)" }));
