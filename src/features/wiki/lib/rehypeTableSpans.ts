...
import { CELL_BG_MARKER_RE, CELL_TH_MARKER_RE, isBgColor } from "../editor/extensions/tableCellColor";
 ...
 * ③ 헤더 열 마커(`{.th}` 접두): 마커를 지우고 td를 th(scope=row)로 바꾼다.
 *
...
export function rehypeTableSpans() {
  return (tree: Root) => {
    visit(tree, "element", (node: Node) => {
      ...
      // 셀 마커 먼저 — 첫 텍스트의 `{.bg-색}`/`{.th}` 접두를 지우고 클래스·태그로 바꾼다(순서 무관)
      for (const tr of rows) {
        for (const cell of cellsOf(tr)) {
          ...
          for (;;) {
            const bg = CELL_BG_MARKER_RE.exec(first.value);
            if (bg && isBgColor(bg[1])) {
              first.value = first.value.slice(bg[0].length);
              const prev = cell.properties?.className;
              const classes = Array.isArray(prev) ? prev : prev ? [prev] : [];
              cell.properties = { ...cell.properties, className: [...classes, `cell-bg-${bg[1]}`] };
              continue;
            }
            const th = CELL_TH_MARKER_RE.exec(first.value);
            if (th) {
              first.value = first.value.slice(th[0].length);
              cell.tagName = "th";
